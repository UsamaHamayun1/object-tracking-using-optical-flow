...
def getFeatures(img, bbox, use_shi=True):
    """
    Extracts features from an image within specified bounding boxes.

    Args:
        img (numpy.ndarray): The input image.
        bbox (numpy.ndarray): The bounding boxes of the objects in the image.
        use_shi (bool, optional): Whether to use the Shi-Tomasi corner detection algorithm.
            Defaults to True.

    Returns:
        numpy.ndarray: The x-coordinates and y-coordinates of the extracted features.
    """
    n_object = len(bbox)
    N = 0
    temp = np.empty(
        (n_object,), dtype=np.ndarray
    )  # temporary storage of x,y coordinates
...
if __name__ == "__main__":
    ...
    for i in range(n_object):
        ...
        patch = Rectangle(
            (xmin, ymin), boxw, boxh, fill=False, color=(0, 0, 0), linewidth=3
        )
        ax.add_patch(patch)
    plt.show()
