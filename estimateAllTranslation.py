...
def estimateAllTranslation(startXs, startYs, img1, img2):
    """
    Estimates the translation of features in img1 to img2 using optical flow.

    Parameters:
    startXs (ndarray): Array of starting X coordinates of features.
    startYs (ndarray): Array of starting Y coordinates of features.
    img1 (ndarray): First image.
    img2 (ndarray): Second image.

    Returns:
    newXs (ndarray): Array of new X coordinates of features in img2.
    newYs (ndarray): Array of new Y coordinates of features in img2.
    """
...
if __name__ == "__main__":
    ...
    for i in range(n_object):
        ...
        patch = Rectangle(
            (xmin, ymin), boxw, boxh, fill=False, color=(0, 0, 0), linewidth=1
        )
        ax.add_patch(patch)
    plt.show()
